...
class Debugger extends \lithium\core\StaticObject
{
    ...
    {
        Filters::apply(Logger::class, 'write', function ($params, $next) {
    ...
    {
        // Response -> Handler
        Filters::apply(Media::class, 'render', function ($params, $next) {
        ...
        Filters::apply(Media::class, '_handle', function ($params, $next) {
            if ($params['handler']['type'] === 'html') {
                Debugger::setView(Media::class, $params['handler'], $params['response']);
            }
            $data           = array();
            $data['start']  = microtime(true);
            $data['memory'] = memory_get_usage(true);
    ...
    public static function initConnections()
    {
        ...
        Filters::apply(Connections::class, '_initAdapter', function ($params, $next) {
            $adapter = $next($params);
            Filters::apply($adapter, array('read', 'create', 'update', 'delete'), function($params, $next) {
    ...
    {
        Filters::apply($controller, '__invoke', function($params, $next) {
            $data            = array();
            $data['start']   = microtime(true);
            $data['memory']  = memory_get_usage(true);
